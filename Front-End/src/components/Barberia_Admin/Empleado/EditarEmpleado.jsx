...
const EditarEmpleado = ({ empleado, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:3001/api/empleados/${empleado.id}`, formData);
      onClose();
    } catch (error) {
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-semibold text-gray-700">Editar Empleado</h3>
        <button 
          onClick={onClose} 
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-4 col-span-1 md:col-span-2">
          <div>
            <label className="block text-gray-600 mb-1">Contraseña (dejar en blanco para mantener la actual)</label>
            <input 
              name="contraseña" 
              type="password" 
              className="w-full border rounded px-3 py-2" 
